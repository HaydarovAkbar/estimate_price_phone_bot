class KeyboardsTexts:
    main = {
        'uz': ["📲 Telefon narxlash 📲💰", "♻️ Tilni o'zgartirish", "👮‍♀️ Adminga murojaat"],
        'ru': ["📲 Оценка телефона 📲💰", "♻️ Изменить язык", "👮‍♀️ Обращение к админу"],
        'en': ["📲 Phone evaluation 📲💰", "♻️ Change language", "👮‍♀️ Appeal to the admin"],
    }

    confirm = {
        'uz': "✅ Tasdiqlash",
        'ru': "✅ Подтвердить",
        'en': "✅ Confirm",
    }
    back = {
        'uz': '🔙 Orqaga',
        'ru': '🔙 Назад',
        'en': '🔙 Back',
    }
    report = {
        'uz': ['Haftalik', 'Oylik', '🔙 Orqaga'],
        'ru': ['Недельный', 'Месячный', '🔙 Назад'],
        'en': ['Weekly', 'Monthly', '🔙 Back'],
    }

    back_main = {
        'uz': '🏠 Bosh Sahifa',
        'ru': '🏠 Главная',
        'en': '🏠 Home',
    }

    languages = ['🇺🇿 O`zbekcha', '🇷🇺 Русский', ]

    sale_product = {
        'uz': ["💸 Sotish", "🏠 Bosh Sahifa"],
        'ru': ["💸 Продать", "🏠 Главная"],
        'en': ["💸 Sell", "🏠 Home"],
    }


class Message:
    main = {
        'uz': "Assalomu alaykum, {}!\n\nBotdan foydalanishingiz mumkin",
        'ru': "Здравствуйте, {}!\n\nВы можете использовать бот",
        'en': "Hello, {}!\n\nYou can use the bot",
    }

    not_admin = {
        'uz': "Adminlar!!\n{}- Bu kanalga botni admin qiling",
        'ru': "Админы!!\n{}- Назначьте бота админом в этом канале",
        'en': "Admins!!\n{}- Make the bot an admin in this channel",
    }
    back = {
        'uz': "🔙 Orqaga",
        'ru': "🔙 Назад",
        'en': "🔙 Back",
    }

    not_found = {
        'uz': "Bunday brand topilmadi",
        'ru': "Такой бренд не найден",
        'en': "Brand not found",
    }

    main_menu = {
        'uz': "Assalomu alaykum, {}!\n\nQuyidagi tillardan birini tanlang:",
        'ru': "Здравствуйте, {}!\n\nВыберите один из следующих языков:",
        'en': "Hello, {}!\n\nChoose one of the following languages:",
    }

    forced_labor = {
        'uz': "Botdan foydalanish uchun kanalimizga obuna bo'ling!",
        'ru': "Подпишитесь на наш канал, чтобы использовать бот!",
        'en': "Subscribe to our channel to use the bot!",
    }

    sale_product = {
        'uz': "📲 Telefonigizni bozordagi narxini bilib olish uchun uning xolatini belgilab chiqing👇🏻",
        'ru': "📲 Узнайте цену своего телефона, указав его состояние👇🏻",
        'en': "📲 Find out the price of your phone by indicating its condition👇🏻",
    }

    get_product = {
        'uz': "{} brendi tanlandi. Ana endi model tanlang👇🏻",
        'ru': "Выбран бренд {}. Теперь выберите модель👇🏻",
        'en': "{} brand selected. Now select the model👇🏻",
    }
    get_capacity = {
        'uz': "{} model tanlandi. Ana endi Capacity (Yomkust) tanlang👇🏻",
        'ru': "Выбрана модель {}. Теперь выберите Capacity (Память)👇🏏",
        'en': "{} model selected. Now select the Capacity👇🏻",
    }

    get_color = {
        'uz': "🎨 Telfoningiz rangini tanlang 👇🏻",
        'ru': "🎨 Выберите цвет вашего телефона 👇🏻",
        'en': "🎨 Choose the color of your phone 👇🏻",
    }

    get_memory = {
        'uz': "📦 Telefonning xotirasini tanlang 👇🏻",
        'ru': "📦 Выберите память телефона 👇🏏",
        'en': "📦 Choose the memory of the phone 👇🏻",
    }

    get_document = {
        'uz': "📦 Telefon Karobkasi bor yoki yo'qligini tanlang 👇🏻",
        'ru': "📦 Выберите наличие коробки у телефона 👇🏻",
        'en': "📦 Choose the presence of a box for the phone 👇🏻",
    }
    success = {
        'uz': "Muvaffaqiyatli bajarildi ✅",
        'ru': "успешно добавлен в админы ✅",
        'en': "successfully added to admins ✅",
    }
    get_country = {
        'uz': "🚩 Telefon ishlab chiqarilgan davlatini tanlang 👇🏻",
        'ru': "🚩 Выберите страну производства телефона 👇🏻",
        'en': "🚩 Choose the country of manufacture of the phone 👇🏏",
    }
    get_status = {
        'uz': "📦 Telefoningizga shikast yetgan yoki yetmaganligini tanlang 👇🏻",
        'ru': "📦 Выберите, был ли ваш телефон разбит 👇🏏",
        'en': "📦 Choose whether your phone was broken 👇🏻",
    }

    choose_language = {
        'uz': "Tilni tanlang",
        'ru': "Выберите язык",
        'en': "Choose the language",
    }

    success_lang_change = {
        'uz': "<code>Til muvaffaqiyatli o'zgartirildi</code>",
        'ru': "<code>Язык успешно изменен</code>",
        'en': "<code>Language successfully changed</code>",
    }

    report_admin = {
        'uz': "<b>Adminga murojaatingizni to'liqroq yozib qoldiring !!!\n\nBiz albatta ko'rib chiqamiz 👮‍♀️</b>",
        'ru': "<b>Оставьте свое обращение админу !!!\n\nМы обязательно рассмотрим 👮‍♀️</b>",
        'en': "<b>Leave your message to the admin !!!\n\nWe will definitely consider it 👮‍♀️</b>",
    }

    help_txt = {
        'uz': """<b>🫵 Ammo siz bizning kanalimiz orqali telefoningizni 15$ qimmatroqga sotishingiz mumkin!

        Kanalga e'lon berish uchun adminga murojaat qiling @PhoneSell_Admin</b>""",
        'ru': """<b>🫵 Но вы можете продать свой телефон дороже нашего канала за 15$!
        
        Обратитесь к админу, чтобы разместить объявление в канале @PhoneSell_Admin</b>""",
        'en': """<b>🫵 But you can sell your phone for more expensive through our channel for 15$!
        
        Contact the admin to place an ad in the channel @PhoneSell_Admin</b>""",
    }

    price_txt = {
        'uz': """<b>@PhoneSell_uz — e'lon berish shartlari:

E'lon narxi 15.000 so'm (narx tushurilmaydi!)
To'lo'v uchun karta raqam 👇

HUMO: - <code>9860060941384087</code> -
Sodiqjonov X (BRB bank)

UzCard: - <code>5614681209209703</code> -
Sodiqjonov X (Ipoteka Bank)

To'lo'v qilib chekini adminga yuboring 👉 @PhoneSell_Admin</b>""",
        'ru': """<b>@PhoneSell_ru — условия размещения объявления:

Стоимость объявления 15.000 сум (цена не указывается!)
Для оплаты номер карты 👇

HUMO: - <code>9860060941384087</code> -
Sodiqjonov X (BRB bank)

UzCard: - <code>5614681209209703</code> -
Sodiqjonov X (Ipoteka Bank)

Оплатите и отправьте чек админу 👉 @PhoneSell_Admin</b>""",
        'en': """<b>@PhoneSell_en — terms of placing an ad:

The cost of the ad is 15.000 sum (price is not indicated!)
For payment card number 👇

HUMO: - <code>9860060941384087</code> -
Sodiqjonov X (BRB bank)

UzCard: - <code>5614681209209703</code> -
Sodiqjonov X (Ipoteka Bank)

Pay and send the check to the admin 👉 @PhoneSell_Admin</b>""",
    }
